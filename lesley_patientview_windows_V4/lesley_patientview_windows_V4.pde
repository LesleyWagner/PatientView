...
final String rpiAddress = "192.168.4.1";
int wifiPacketCounter = 0;
byte delimiter = 0x00;
final int dataLength = 8;
byte[] newDataPacket; // new data from raspberry pi
boolean connected = false; // connected to raspberry pi?

/************** GUI variables (Lesley) **********************/
Textlabel lblPi;
...
public void setup() {   
  if (testing) {
    ...
    client = new Client (this, rpiAddress, port);
  ...
  if (!testing) {
    // synchronize raspberry pi data
    int dataByte = 0xFF;
    while (dataByte != delimiter) {
...
/**
 * process sensor data from Raspberry Pi 3B sent over WIFI
 */
public void processPiData() {
  byte[] dataPacket = new byte[10]; // data packet with 10 bytes
  while (client.available() >= dataLength+2 && (client.readBytesUntil(delimiter, dataPacket) == dataLength+2)) {
...
public void connectToPi() {
  if (!connected) {
    try {
      InetAddress ipAddress = InetAddress.getByName(rpiAddress);
      if (ipAddress.isReachable(1000)) {
        Socket piSocket = new Socket (ipAddress, port);
        client = new Client (this, piSocket);
        connected = true;
        lblPi.setText("connected to Pi"); 
      }
      else {      
        println("server not found");
        // to do print message on the screen
      }
    } 
    catch (Exception e) {
