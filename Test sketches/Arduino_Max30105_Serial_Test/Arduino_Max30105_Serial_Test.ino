...
// serial data transmit variables 
...
void loop()
{
  ...
  while (1)
  {
    ...
    for (byte i = 75; i < 100; i++)
    {
      ...
      dataPacket[1] = redBuffer[i] >> 8; // red MSB
      dataPacket[2] = redBuffer[i]; // red LSB
      dataPacket[3] = irBuffer[i] >> 8; // ir MSB
      dataPacket[4] = irBuffer[i]; // ir LSB
      dataPacket[5] = (uint8_t)heartRate;
      dataPacket[6] = (uint8_t)spo2;
      dataPacket[7] = 1;
      dataPacket[8] = 119;     
...
//      Serial.println("");
      ...
      // transmit data
      Serial.write(dataPacket, dataLength);
    }
